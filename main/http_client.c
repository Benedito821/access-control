...
static void http_rest_with_url(void)
{
    ...
    // //DELETE
    // esp_http_client_set_url(client, "http://"HTTP_ENDPOINT"/delete");
    ...
    // //HEAD
    // esp_http_client_set_url(client, "http://"HTTP_ENDPOINT"/head");
...
{
    esp_http_client_config_t config = {
        .url = "http://"HTTP_ENDPOINT"/relative-redirect/3",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
...
{
    esp_http_client_config_t config = {
        .url = "http://"HTTP_ENDPOINT"/absolute-redirect/3",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
...
{
    esp_http_client_config_t config = {
        .url = "http://"HTTP_ENDPOINT"/redirect-to?url=https://www.howsmyssl.com",
        .event_handler = _http_event_handler,
        .cert_pem = howsmyssl_com_root_cert_pem_start,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
...
{
    esp_http_client_config_t config = {
        .url = "http://"HTTP_ENDPOINT"/stream-bytes/8912",
        .event_handler = _http_event_handler,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_err_t err = esp_http_client_perform(client);
...
static void https_async(void)
{
    ...
    config.url = "https://"HTTP_ENDPOINT"/get";
...
static void http_client_task(void *pvParameters)
{
    for(;;)
    {
        if(is_wifi_con_up == true)
        {
           //Todo:  ping the server first https://github.com/nopnop2002/esp-idf-ping
            http_rest_with_url();
...
{
    xTaskCreate(&http_client_task, "http_client_task", HTTP_CLIENT_TASK_STACK_DEPTH, NULL, HTTP_CLIENT_TASK_PRIO, NULL);
}